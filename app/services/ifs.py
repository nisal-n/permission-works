...
from app.settings import get_active_env, get_env, get_default_name
...
def _is_env_configured(block: dict | None) -> bool:
    block = block or {}
    return bool(
        (block.get("base_root") or block.get("cfg_base_root") or "").strip()
        and (block.get("token_url") or block.get("cfg_token_url") or "").strip()
        and (block.get("client_id") or block.get("cfg_client_id") or "").strip()
    )


def _resolve_permission_env_block(preferred_name: str = "DEV") -> dict:
    """
    Resolve the connection details that permission APIs should use.

    Priority:
      1) default environment selected in Setup IFS
      2) explicitly named environment (usually DEV)
      3) active mapped environment from settings
    """
    names = []
    try:
        default_name = (get_default_name() or "").strip()
    except Exception:
        default_name = ""
    if default_name:
    ...
    for name in names:
        ...
        seen.add(name)
        try:
            block = get_env(name) or {}
        except Exception:
            block = {}
        if _is_env_configured(block):
            return block

    try:
        active = get_active_env(preferred_name) or {}
    except Exception:
        active = {}
    if _is_env_configured(active):
        return active

    return get_env(preferred_name) or active or {}


def _dev_conf() -> dict:
    """
    Resolve permission-service connection details from the Setup IFS default
    environment first, then fall back to DEV. This keeps create/update/fetch/
    grants on the same tenant instead of mixing a token from one env with a
    hard-coded or stale base URL from another.
    """
    s_env = _resolve_permission_env_block("DEV")
    env = os.environ

    base_root = (s_env.get("base_root")
                 or s_env.get("cfg_base_root")
                 or env.get("DEV_BASE_ROOT")
                 or current_app.config.get("DEV_BASE_ROOT", "")
                 ).rstrip("/")

    token_url = (s_env.get("token_url")
                 or s_env.get("cfg_token_url")
                 or env.get("DEV_TOKEN_URL")
                 or current_app.config.get("DEV_TOKEN_URL", "")
                 ).strip()

    client_id = (s_env.get("client_id")
                 or s_env.get("cfg_client_id")
                 or env.get("DEV_CLIENT_ID")
                 or current_app.config.get("DEV_CLIENT_ID", "")
                 ).strip()

    client_secret = (s_env.get("client_secret")
                     or s_env.get("cfg_client_secret")
                     or env.get("DEV_CLIENT_SECRET")
                     or current_app.config.get("DEV_CLIENT_SECRET", "")
                     ).strip()

    scope = (s_env.get("scope")
             or s_env.get("cfg_scope")
             or env.get("DEV_SCOPE")
             or current_app.config.get("DEV_SCOPE", "openid microprofile-jwt")
             ).strip()

    ssl_verify = bool(
        s_env.get("ssl_verify", s_env.get("cfg_ssl_verify", current_app.config.get("DEV_SSL_VERIFY", True)))
    )
...
def get_access_token() -> str | None:
    ...
    try:
        r = requests.post(c["token_url"], data=data, timeout=30, verify=c["ssl_verify"])
        r.raise_for_status()
...
def get_permission_set_service_base() -> str:
    conf = _dev_conf()
    base = (conf.get("projection_base") or "").rstrip("/")
    return f"{base}/PermissionSetHandling.svc" if base else ""


def get_permission_set_url() -> str:
    base = get_permission_set_service_base().rstrip("/")
    return f"{base}/PermissionSets" if base else ""

def get_permission_ssl_verify() -> bool:
    conf = _dev_conf()
    return bool(conf.get("ssl_verify", True))
...
def fetch_permission_sets(token, top=200, q=None):
    base = get_permission_set_url().rstrip("/")
    verify = get_permission_ssl_verify()
    ...
    try:
        r = requests.get(url, headers=_std_headers(token), timeout=30, verify=verify)
        if r.status_code != 200:
            return []
        data = r.json()
...
def fetch_users_page(token, top=20):
    try:
        base = f"{get_permission_set_service_base().rstrip('/')}/Users"
        url = f"{base}?$top={int(top)}"
        r = requests.get(url, headers=_std_headers(token), timeout=30, verify=get_permission_ssl_verify())
        if r.status_code != 200:
...
def fetch_user_groups_page(token, top=20):
    try:
        base = f"{get_permission_set_service_base().rstrip('/')}/UserGroups"
        url = f"{base}?$top={int(top)}"
        r = requests.get(url, headers=_std_headers(token), timeout=30, verify=get_permission_ssl_verify())
        if r.status_code != 200:
...
# app/services/ifs.py
def grant_groups(role, grantees, token=None):
    """
    Grant a role to user groups (DEV env), accepting either:
      - a list of group IDs: ["AAA","BBB",...]
      - a pre-encoded grantees string (e.g., "USER_GROUP_ID=A^;USER_GROUP_ID=B^")
      - or legacy variants like "GROUP_IDENTITY=A^;..."
    This helper performs the required 'AddGroupsVirtuals' before 'GrantGroups'.

    Returns:
      - int (count) if a list was passed
      - True/False if a string was passed
    """
    import requests

    if not role:
        return 0 if isinstance(grantees, (list, tuple)) else False
    ...
    base   = _dev_conf()["projection_base"].rstrip("/")
    verify = _dev_conf().get("ssl_verify", True)
    ...
    # Normalize input to the caret-delimited format IFS expects for groups
    # Preferred key is USER_GROUP_ID=...^
    if isinstance(grantees, (list, tuple)):
        group_ids = [g for g in grantees if isinstance(g, str) and g.strip()]
        grantees_str = "".join(f"USER_GROUP_ID={g.strip()}^" for g in group_ids)
        want_count = len(group_ids)
    else:
        s = (grantees or "").strip()
        # Accept GROUP_IDENTITY=...^ and normalize to USER_GROUP_ID=...^
        if "GROUP_IDENTITY=" in s and "USER_GROUP_ID=" not in s:
            s = s.replace("GROUP_IDENTITY=", "USER_GROUP_ID=")
        # Also accept comma/semicolon separated bare IDs and encode them
        if "^" not in s and "=" not in s:
            parts = [p.strip() for p in re.split(r"[;,]", s) if p.strip()]
            s = "".join(f"USER_GROUP_ID={p}^" for p in parts)
        grantees_str = s
        want_count = None  # we'll return True/False in this branch

    # 1) Seed virtual (required by the backend flow)
    add_url = f"{base}/PermissionSetHandling.svc/AddGroupsVirtuals"
    r = requests.post(add_url, json={"Role": role}, headers=headers, timeout=30, verify=verify)
    if r.status_code not in (200, 201, 204):
        return 0 if want_count is not None else False

    # 2) Grant
    grant_url = f"{base}/PermissionSetHandling.svc/GrantGroups"
    payload = {"Role": role, "Grantees": grantees_str}
    r2 = requests.post(grant_url, json=payload, headers=headers, timeout=60, verify=verify)
    ok = r2.status_code in (200, 201, 204)

    if want_count is not None:
        return want_count if ok else 0
    return ok
...
__all__ = [
    ...
    "get_permission_set_service_base", "get_permission_set_url", "get_permission_ssl_verify",
    # orchestrator
    "export_and_import",
]
...
# === Grant-by-Command (Commands bucket only) ===============================

def _ifs_headers(token: str) -> dict:
    # OData headers that work with PermissionSetHandling.svc
    return {
        "Authorization": f"Bearer {token}",
...
def _psh_base_url() -> str:
    """
    Use the same DEV projection base the rest of this module uses.
    This prevents '/PermissionSetHandling.svc/...' MissingSchema errors.
    """
    c = _dev_conf()
    base = (c.get("projection_base") or "").rstrip("/")
    if not base:
        raise RuntimeError(
            "DEV projection_base is empty. Configure DEV base_root in Settings or DEV_BASE_ROOT env."
        )
    return f"{base}/PermissionSetHandling.svc"



def grant_projection_commands(role: str, projection: str, commands: list[str]) -> tuple[bool, dict]:
    """
    Grants all *Commands-group* entity action grants for the given command names in `projection`
    to the given `role`, using ManageGrantsByCommandAssistantVirtual.

    IMPORTANT:
      • Client is derived server-side from Reference_ActionCommand (DO NOT trust UI's "client").
      • Strict If-Match with fresh ETag for every bound action.
      • We reset to REVOKE, then SelectCommands, then ApplyChanges (IFS current contract).
    """
    ...
    if not role or not projection or not commands:
        return False, {"error": "role/projection/commands are required"}

    # keep only non-empty strings
    commands = [x for x in (commands or []) if isinstance(x, str) and x.strip()]
    if not commands:
        return False, {"error": "no commands to grant"}

    token = get_access_token() if "get_access_token" in globals() else None
    if not token:
        return False, {"error": "no access token"}

    base = _psh_base_url()
    # safety: prevent MissingSchema if base misconfigured
    if not base or base.startswith("/"):
        return False, {"where": "config", "error": "invalid projection base url", "got": base}

    H = _ifs_headers(token)

    # 1) Create assistant (or reuse existing one server-side)
    r = requests.post(
    ...
    if not objkey:
        # Some tenants only expose ETag/object after an explicit GET
        # (leaving as-is to avoid logic changes)
        pass
    ...
    if g.status_code != 200:
        return False, {"where": "get_assistant", "status": g.status_code, "text": (g.text or "")[:800]}
    etag = g.headers.get("ETag") or g.headers.get("Etag") or g.headers.get("etag")

    def with_if_match(h: dict) -> dict:
        return {**h, **({"If-Match": etag} if etag else {})}

    # 3) Reset everything in this projection to REVOKE (per IFS recommended flow)
    for action in (
        "ManageGrantsByCommandAssistantVirtual_SetAllProjectionActionsToRevoke",  # primary
        "ManageGrantsByCommandAssistantVirtual_SetAllToRevoke",                   # fallback
        "ManageGrantsByCommandAssistantVirtual_SetAllActionsToRevoke",            # fallback
    ):
        url = f"{entity_url}/IfsApp.PermissionSetHandling.{action}"
        rr = requests.post(url, json={}, headers=with_if_match(H), timeout=30, verify=verify)
        if 200 <= rr.status_code < 300:
            etag = rr.headers.get("ETag") or etag
            break
    else:
        return False, {
            "where": "set_all_to_revoke",
            "status": rr.status_code if 'rr' in locals() else None,
            "text": (getattr(rr, 'text', '') or '')[:1200]
        }

    # 4) Read Reference_ActionCommand to get the TRUE client for each command
    q_proj = projection.replace("'", "''")
    ref_url = f"{base}/Reference_ActionCommand?$select=Command,Client&$filter=ProjectionName eq '{q_proj}'"
    rmap = requests.get(ref_url, headers=H, timeout=30, verify=verify)
    if rmap.status_code != 200:
        return False, {"where": "read_ref_commands", "status": rmap.status_code, "text": (rmap.text or "")[:1000]}

    rows = (rmap.json() or {}).get("value", [])
    cmd_to_clients = {}
    for row in rows:
        cmd = (row or {}).get("Command") or ""
        cli = (row or {}).get("Client") or ""
        if cmd and cli:
            cmd_to_clients.setdefault(cmd, []).append(cli)

    missing = [cmd for cmd in commands if cmd not in cmd_to_clients]
    if missing:
        return False, {
            "where": "select_commands",
            "error": "command not found in Reference_ActionCommand",
            "projection": projection,
            "missing": missing
        }

    # Choose a client per command (prefer 'web' if present; else the first advertised client)
    pairs = []
    for cmd in commands:
        clients = cmd_to_clients.get(cmd, [])
        chosen = "web" if "web" in clients else (clients[0] if clients else None)
        if not chosen:
            return False, {"where": "select_commands", "error": f"no client found for {cmd}"}
        pairs.append((chosen, cmd))

    selection = ";".join(
        f"CLIENT={cl}^COMMAND={cmd}^PROJECTION_NAME={projection}^"
        for (cl, cmd) in pairs
    )

    # 5) SelectCommands
    sel_url = f"{entity_url}/IfsApp.PermissionSetHandling.ManageGrantsByCommandAssistantVirtual_SelectCommands"
    rs = requests.post(sel_url, json={"Selection": selection}, headers=with_if_match(H), timeout=30, verify=verify)
    if not (200 <= rs.status_code < 300):
        return False, {"where": "select_commands", "status": rs.status_code, "text": (rs.text or "")[:1500]}
    ...
    ra = requests.post(apply_url, json={}, headers=with_if_match(H), timeout=30, verify=verify)
    ...
    return True, {
        "role": role,
        "projection": projection,
        "granted_commands": sorted(set(commands)),
        "selection": selection,
    }


def grant_projection_command(role: str, projection: str, command: str) -> tuple[bool, dict]:
    """
    Single-command wrapper. Intentionally **ignores** any 'client' provided by callers/UI.
    We ALWAYS derive the correct client from Reference_ActionCommand.
    """
    return grant_projection_commands(role=role, projection=projection, commands=[command])
