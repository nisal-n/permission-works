...
def _project_root() -> str:
    return app.root_path and str(__import__("pathlib").Path(app.root_path).parent)


def _recordings_dir() -> str:
    d = __import__("os").path.join(_project_root(), "recordings")
...
@tracing_bp.route("/api/start_trace_session", methods=["POST"])
@login_required
def start_trace_session():
    """
    Begin a trace session without a redirect so the screen-recording flow can
    finish its own UI state changes before the user moves on.
    """
    from .. import state
    ...
    state.reports.clear()
    return jsonify({"ok": True, "is_tracing": True})
...
def start_trace():
    ...
    state.workflows.clear()
    state.reports.clear()
    autolaunch = (request.args.get("autolaunch") or "").strip() == "1"
    if autolaunch:
        return redirect(url_for("tracing.view_traces", autolaunch=1))
...
@tracing_bp.route("/api/save_screen_recording", methods=["POST"])
@login_required
def save_screen_recording():
    ...
    role = (request.form.get("role") or session.get("update_role") or "unknown").strip()
    mode = (request.form.get("mode") or ("update" if session.get("update_role") else "create")).strip() or "trace"
    ...
    filename = secure_filename(f"{mode}_{role}_{ts}{ext}") or f"recording_{ts}{ext}"
